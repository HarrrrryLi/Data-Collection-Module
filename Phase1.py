...
Stocks=['APPL','GOOGL','NVDA','YHOO','AMZN']
...
try:
    ...
    def get_price_RT(sym):
        ...
        t= pd.DataFrame({'time':Time_now,'sym': sym,'price': [Share(sym).get_price()], 'volume': [Share(sym).get_volume()]},index=[Time_now])
        return t
    ...
    frames = [df_apple, df_google, df_nvidia, df_yahoo, df_amazon]
    for i in range(0,5):
        result = frames[i]
